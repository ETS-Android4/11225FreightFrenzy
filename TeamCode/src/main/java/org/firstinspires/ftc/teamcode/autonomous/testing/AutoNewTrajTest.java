...
import org.firstinspires.ftc.teamcode.competition.util.NewBlueTrajectoryGenerator;
 ...
 */
@Disabled
@Autonomous(name="Auto traj", preselectTeleOp="TeleOp final blue")
public class AutoNewTrajTest extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        drive.followTrajectory(trajs.get(0)); // going to shipping hub
        ...
        if (startPosition == StartPosition.FRONT) {
            drive.followTrajectory(trajs.get(1)); // going to duck wheel
            sleep(300);
        ...
        telemetry.update();
        drive.followTrajectory(trajs.get(2)); // going to park in warehouse
    ...
    private ArrayList<Trajectory> generateTrajs(SampleMecanumDrive drive, Color color, StartPosition startPosition, ParkingMethod parkingMethod) {
        ArrayList<Trajectory> trajs;
        if (color == Color.RED) {
...
//            trajs = generator.generateTrajectories();
            throw new NotImplementedError("I'm testing moron");
        } else {
            NewBlueTrajectoryGenerator generator = new NewBlueTrajectoryGenerator(drive, startPosition, parkingMethod);
            trajs = generator.generateTrajectories();
        }
        return trajs;
    }
