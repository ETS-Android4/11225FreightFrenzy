...
public class TeleopTestBlueSide extends LinearOpMode {
    ...
    public void peripheralMove(){
        ...
        // TSE Arm
        if (ControlConfig.toggleTseArmServo && unpressed) {
            tseArmActive = !tseArmActive;
            robot.tseArmServo.setPosition(tseArmActive ? Constants.tseArmActivePosition : Constants.tseArmInitPosition);
            unpressed = false;
        } else if (ControlConfig.toggleTseRodServo && unpressed) {
            tseRodActive = !tseRodActive;
            robot.tseRodServo.setPosition(tseRodActive ? Constants.tseRodActivePosition : Constants.tseRodInitPosition);
            unpressed = false;
        } else if (!(ControlConfig.toggleTseArmServo && ControlConfig.toggleTseRodServo)){
            unpressed = true;
        }
    }
