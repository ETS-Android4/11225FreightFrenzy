...
public class AutoInputTest extends LinearOpMode {
    ...
    //Telemetry telemetry;
    OpenCvCamera webcam;
    double towerWheelSpeed = Constants.towerWheelSpeed;
...
//        telemetry.update();
        ...
        if (color == Color.RED) {
            towerWheelSpeed=-Constants.towerWheelSpeed;
        ...
        if (position == Position.FRONT) {
            ...
            sleep(300);
            // TODO deliver duck
            robot.towerMotor.setPower(towerWheelSpeed);
            sleep(2500);
    ...
    private void dumpPreloaded() {
        ...
        while (opModeIsActive() && robot.liftMotor.isBusy()) {
            telemetry.addLine("got to lin slide move");
            telemetry.update();
        }
